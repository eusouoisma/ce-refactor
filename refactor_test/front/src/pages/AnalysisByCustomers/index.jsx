...
export default function AnalysisByCustomers() {
  const [filters, setFilters] = useState({ startDate: '', endDate: '', clientSearch: '', orderBy: 'pax', from: 0, to: 100 });
  const [data, setData] = useState([]);

  function set(f, v) { setFilters(p => ({ ...p, [f]: v })); }

  async function search() {
    const res = await fetch(`${API_URL}/reports/analysis-by-customers`, {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h5" sx={{ mb: 2, fontWeight: 'bold' }}>Análise por Cliente</Typography>
      <Card sx={{ mb: 2 }}>
        <CardContent>
          <Grid container spacing={2} alignItems="center">
            ...
            <Grid item xs={3}><TextField fullWidth size="small" label="Pesquisar Cliente" value={filters.clientSearch} onChange={e => set('clientSearch', e.target.value)} /></Grid>
            <Grid item xs={2}>
            ...
            <Grid item xs={1}><Button variant="contained" onClick={search}>Buscar</Button></Grid>
          </Grid>
        </CardContent>
      </Card>
      <TableContainer component={Paper}>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell>#</TableCell><TableCell>Cliente</TableCell><TableCell>Moeda</TableCell>
          ...
          <TableBody>
            {data.map((r, i) => (
              <TableRow key={i}>
                <TableCell>{r.index}</TableCell><TableCell>{r.client}</TableCell><TableCell>{r.currency}</TableCell>
