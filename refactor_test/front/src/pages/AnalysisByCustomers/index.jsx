...
const PIE_COLORS = [
  ...
  '#16A2D7','#FF6D01','#46BDC6','#EA1E63','#00BFA5',
];
...
export default function AnalysisByCustomers() {
  ...
  useEffect(() => {
    const t = setTimeout(() => setDebounced(clientSearch), 400);
    return () => clearTimeout(t);
  }, [clientSearch]);

  useEffect(() => {
    setLoading(true);
    apiFetch('/reports/analysis-by-customers', {
      method: 'POST',
      body: JSON.stringify({ startDate, endDate, orderBy, from, to, clientSearch: debounced }),
    })
      .then(r => r.json())
      .then(d => setData(Array.isArray(d) ? d : []))
      .catch(() => setData([]))
      .finally(() => setLoading(false));
  }, [startDate, endDate, orderBy, from, to, debounced]);
  ...
  const bgColors = data.map((_, i) => PIE_COLORS[i % PIE_COLORS.length]);
  const piePax = {
    labels: data.map(d => d.client || 'N/A'),
    datasets: [{ data: data.map(d => d.totalPax), backgroundColor: bgColors, borderWidth: 1 }],
  };
  const pieValor = {
    labels: data.map(d => d.client || 'N/A'),
    datasets: [{ data: data.map(d => parseFloat(d.valorTotal)), backgroundColor: bgColors, borderWidth: 1 }],
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="h5" fontWeight={700} sx={{ mb: 2.5 }}>Análise por Cliente</Typography>
      ...
      <Paper variant="outlined" sx={{ p: 2.5, mb: 2.5 }}>
        <Grid container spacing={2} alignItems="flex-end">
          ...
          <Grid item xs={12} sm={2.5}>
            <TextField fullWidth size="small" label="Pesquisar Cliente" value={clientSearch}
              onChange={e => setClientSearch(e.target.value)} placeholder="Digite o nome do cliente..." />
          </Grid>
            ...
            <Table size="small" stickyHeader>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={thSx}>Cliente</TableCell>
              ...
              <TableBody>
                {data.length === 0 ? (
                  ...
                  <TableRow key={i} sx={{ '&:hover td': { bgcolor: COLORS.tableHover } }}>
                    ...
                    <TableCell sx={cellSx(i)}>{row.client || 'N/A'}</TableCell>
