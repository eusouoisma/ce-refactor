...
import { Main, SubTitle, Title } from "./analysisbycustomers";
...
const AnalysisByCustomers = () => {
    ...
    // Debounce para pesquisa de cliente (400ms de delay)
    useEffect(() => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await fetch(`${API_URL}reports/analysis-by-customers.php`, {
                    method: "POST",
                    headers: {
        ...
        fetchData();
    }, [startDate, endDate, orderBy, from, to, debouncedClientSearch]);
    ...
    const pieValor = {
        ...
        datasets: [{
            label: "Valor Total",
            data: data.map(d => parseFloat(d.valorTotal)),
            backgroundColor: [
                '#4285F4', '#EA4335', '#FBBC05', '#34A853', '#8E24AA',
                '#16A2D7', '#FF6D01', '#46BDC6', '#EA1E63', '#00BFA5'
            ],
                                        ...
                                        {!isOnlyReportsUser && (
                                            <td>
                                                {Number(row.valorTotal).toLocaleString("pt-BR", {
                                                    style: "currency",
                                                    currency: ["BRL", "USD", "EUR"].includes(row.currency)
                                                        ? row.currency
                                                        : "BRL",
                                                })}
                                            </td>
...
export default AnalysisByCustomers;
